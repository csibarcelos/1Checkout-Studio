...
export const AuthPage: React.FC = () => {
  ...
  const { login, register, isAuthenticated, isLoading: authContextLoading } = useAuth();
    ...
    <div className="min-h-screen bg-neutral-900 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center">
        ...
        <p className="mt-2 text-center text-sm text-neutral-400">
          Ou{' '}
          <button
            onClick={() => { setIsRegisterView(!isRegisterView); setError(null); }}
            className="font-medium text-primary hover:text-primary-dark"
          >
            {isRegisterView ? 'faça login na sua conta existente' : 'crie uma nova conta gratuitamente'}
          </button>
        </p>
      </div>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <Card className="bg-neutral-800 shadow-2xl border-neutral-700/50">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              {/* AuthPage specific button still uses bg-primary, text-neutral-900 for high impact */}
              <Button 
                type="submit" 
                className="w-full bg-primary text-neutral-900 hover:bg-primary-dark focus:ring-primary" // Explicitly keep yellow background here
